  ...
  const trackCols = React.useMemo(
    () =>
      Array.from({ length: COLUMN_COUNT }, (_, i) => {
        ...
        return {
          left: Number(((i + 0.5) * (100 / COLUMN_COUNT)).toFixed(6)),
          opacity: Number((0.008 + rng() * 0.012).toFixed(6)),
        };
      }),
    [],
  );
  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes heroIconBlink {
          0%, 65%, 100% { opacity: var(--b0, 0.03); }
          82%           { opacity: var(--b1, 0.055); }
        }
        ...
        <div className="absolute inset-0">
          {rain.map((t) => {
            const leftPct = Number(
              ((t.col + 0.5) * (100 / COLUMN_COUNT)).toFixed(6)
            );
            const offsetXPx = Number((t.offsetX / 12).toFixed(6));
            const topStartRounded = Number(t.topStart.toFixed(6));
            const rotateRounded = Number(t.rotate.toFixed(6));
            const sizePxRounded = Number(t.sizePx.toFixed(6));
            const durationRounded = Number(t.duration.toFixed(6));
            const delayRounded = Number(t.delay.toFixed(6));
            const opacityRounded = Number(t.opacity.toFixed(6));

            const leftVal =
              offsetXPx >= 0
                ? `calc(${leftPct}% + ${offsetXPx}px)`
                : `calc(${leftPct}% - ${Math.abs(offsetXPx)}px)`;
            ...
            return (
              <div
                ...
                style={
                  {
                    left: leftVal,
                    transform: `translate3d(0, ${topStartRounded}%, 0) rotate(${rotateRounded}deg)`,
                    color: TOKEN_COLORS[t.color],
                    opacity: 0,
                    fontSize: `${sizePxRounded}px`,
                    fontWeight: fontVar,
                    fontFamily: t.mono
                      ? "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace"
                      : "inherit",
                    letterSpacing: t.mono ? "0.01em" : "0",
                    ["--op" as string]: `${opacityRounded}`,
                    ["--rot" as string]: `${rotateRounded}deg`,
                    animation: !effectiveReduce
                      ? `heroCodeRainFall ${durationRounded}s linear ${delayRounded}s infinite`
                      : "none",
        ...
        <div className="absolute inset-0">
          {LANG_ICONS.map((icon, i) => {
            ...
            const floatAni = !effectiveReduce
              ...
              : "none";
            const blinkAni = !effectiveReduce
              ? `heroIconBlink ${BLINK_CYCLE}s ease-in-out ${blinkDelay}s infinite`
              : "none";
            return (
              <div
                key={`li-${i}`}
                ...
                style={
                  {
                    ...
                    ["--b0" as string]: `${icon.opacity}`,
                    ["--b1" as string]: `${blinkBoost}`,
                    animation:
                      floatAni === "none" && blinkAni === "none"
                        ? "none"
                        : [floatAni, blinkAni].filter((a) => a !== "none").join(", "),
                  } as React.CSSProperties
                }
              >
                {icon.svg}
      ...
      <Container size="xl" className="relative w-full">
        <div className="mx-auto grid min-h-[540px] sm:min-h-[620px] lg:min-h-[540px] grid-cols-1 items-center gap-8 sm:gap-10 lg:grid-cols-12 lg:gap-12 xl:gap-16 py-[60px] sm:py-[70px] lg:py-[70px] pb-16 sm:pb-20">
          ...
          <motion.div
            ...
            animate={state}
            custom={0}
            className="relative order-2 w-full max-w-full text-left lg:order-1 lg:col-span-6 xl:col-span-6 lg:pr-2 xl:pr-4"
          >
            <motion.p
              variants={fadeUp}
              initial="offscreen"
            ...
            <motion.h1
              ...
              className="text-[38px] sm:text-[46px] md:text-[58px] font-bold leading-[1.10] text-[#172B48] text-balance"
            >
               Learn the Digital
              <br />
              Skills That{" "}
            ...
            <motion.p
              ...
              animate={state}
              custom={STAGGER.body}
              className="mt-6 mb-8 w-full max-w-[100%] lg:max-w-[580px] text-[17px] sm:text-[18px] font-normal leading-[29px] text-[#4A5568] text-pretty"
            >
            ...
            >
              <Link
                href="/courses"
                className="group/cta group/btn relative inline-flex h-12 sm:h-13 cursor-pointer items-center gap-2 overflow-hidden rounded-lg bg-gradient-to-r from-[#172B48] to-[#222F5D] px-7 sm:px-8 text-white shadow-[0_8px_24px_-10px_rgba(23,43,72,0.55)] ring-1 ring-[#222F5D]/80 transition-all duration-300 hover:-translate-y-[1px] hover:shadow-[0_12px_32px_-10px_rgba(23,43,72,0.7)] active:translate-y-[1px]"
              >
                <span className="relative z-10 text-[14.5px] sm:text-[15.5px] font-semibold tracking-[0.01em]">
                  Browse Courses
                </span>
                <svg
                  aria-hidden="true"
                  className="relative z-10 h-[18px] w-[18px] -mr-0.5 translate-x-0 transition-transform duration-300 group-hover/cta:translate-x-[2px]"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2.2"
              ...
              >
                <span className="relative grid h-12 w-12 shrink-0 place-items-center rounded-full bg-white text-[#22C7E8] shadow-[0_8px_24px_-8px_rgba(34,199,232,0.55)] ring-1 ring-[#22C7E8]/25 transition-all duration-300 group-hover/play:scale-105 group-hover/play:ring-[#22C7E8]/60">
               <MessageCircle className="size-5" aria-hidden="true" />
                </span>
                <span className="flex flex-col items-start leading-tight">
                  <span className="text-[15px] font-semibold text-[#172B48]">
          ...
          <motion.div
            ...
            className="relative order-1 mx-auto w-full max-w-[480px] sm:max-w-[560px] lg:order-2 lg:col-span-6 xl:col-span-6 lg:max-w-none"
          >
            <div className="relative mx-auto aspect-[425/440] w-full max-w-[480px] sm:max-w-[560px] max-h-[450px] sm:max-h-[470px] lg:max-h-[500px] lg:max-w-none">
              <Image
                src={HERO_IMG}
                alt="Hero image"
                fill
              ...
              <FloatWrap
                ...
                floatY={[-8, 10]}
                floatSec={7}
                className="absolute left-0 top-[37%] sm:top-[36%] lg:top-[37%] w-[38%] sm:w-[38%] h-[40%]"
              >
                <Image
                  src={POPUP_STUDENTS}
